...
class MDN(tf.keras.layers.Layer):
    ...
    def __init__(self, output_dimension, num_mixtures, **kwargs):
        ...
        with tf.name_scope('MDN'):
            self.mdn_mus = tf.keras.layers.Dense(self.num_mix * self.output_dim, name='mdn_mus')  # mix*output vals, no activation
            self.mdn_sigmas = tf.keras.layers.Dense(self.num_mix * self.output_dim, activation=elu_plus_one_plus_epsilon, name='mdn_sigmas')  # mix*output vals exp activation
    ...
    def call(self, x, mask=None):
        with tf.name_scope('MDN'):
            mdn_out = tf.keras.layers.concatenate([self.mdn_mus(x),
                                          self.mdn_sigmas(x),
                                          self.mdn_pi(x)],
                                         name='mdn_outputs')
        return mdn_out
